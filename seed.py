...
# ADD USERS
    ...
    db.session.commit()
    # ADD EVENTS
    e1 = Event(contact_id=ivw.id, date=datetime.datetime(2017, 12, 30))
    e2 = Event(contact_id=rec.id)
    e3 = Event(contact_id=ivw.id)
    e4 = Event(contact_id=sec.id, date=datetime.datetime(2018, 1, 1))
    db.session.add_all([e1, e2, e3, e4])
    db.session.commit()
    # ADD TEMPLATES
    fup = Template(name='follow up', text='hello there', event_id=e1.id)
    ty = Template(name='thank you', text='thank you for meeting!', event_id=e2.id)
    ty2 = Template(name='thank you', text='thank you for meeting!', event_id=e3.id)
    fup2 = Template(name='follow up', text='hello there', event_id=e4.id)
